# 分群演算法：Greedy Modularity Communities
...
def generate_social_network_analysis_and_json(adj_df, recip_df, metrics_df):
    ...
    ax.set_title(f"網紅社群勢力圖 (Greedy Modularity 分群 | 已移除 {len(isolated_nodes)} 位無連結網紅)", fontsize=36, pad=50, weight='bold', loc='center')
    ...
    ax.legend(handles=legend_handles, title="社群領袖 (Greedy - 依圈內影響力)", loc='upper right', fontsize=16)
    ...
    if isolated_nodes:
        comm_rows.append({'派系名稱': '0-Degree', '成員總數': len(isolated_nodes), '核心領袖': isolated_nodes[0], '所有成員': " | ".join(isolated_nodes)})
